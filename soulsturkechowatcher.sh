...
PENDING_MSG_FILE="/tmp/soulsturkechowatcher.pending_msg"   # FIX: Kopuş mesajı kuyruğu
...
# FIX: Bağlantı geri gelince bekleyen kopuş mesajını gönder
flush_pending_msg() {
...
if [ "$1" = "--daemon" ]; then
    ...
    # FIX: Daemon başlarken eski geçici dosyaları temizle
    rm -f "$STATE_FILE" "$PENDING_MSG_FILE"
        ...
        logread -f 2>/dev/null | while read -r line; do
            case "$line" in
                ...
                *"No response to 3 echo-requests"*|*"LCP terminated"*)
                    # Sadece PPPoE echo-request kaynaklı kopmalar — genel internet kesintisi değil
                    if [ ! -f "$PENDING_MSG_FILE" ]; then
                        date +%s > "$STATE_FILE"
                        TIME_NOW=$(date "+%d.%m.%Y %H:%M:%S")
                        if echo "$line" | grep -q "No response to 3 echo-requests"; then
                            REASON="ISS tarafından 3 Echo yanıtı gelmediği için oturum yenilendi."
                        else
                    ...
                    ;;
                *"Internet access detected"*|*"Internet access restored"*)
...
🔄 IP Durumu: $IP_CHANGE"
    ...
    while true; do
        # FIX: tail -n 20 → tail -n 50 (daha fazla satır izle)
        current_logs=$(ndmc -c 'show log' 2>/dev/null | tail -n 50)
...
INITEOF
        ...
        9)
            ...
            if [ "$confirm" = "EVET" ]; then
                ...
                # FIX: Geçici dosyaları da temizle
                rm -f "$STATE_FILE" "$PENDING_MSG_FILE" "$LAST_LOG_FILE" "$LAST_IP_FILE"
